...
const initialPosts = [
  {
    title: "РАЗРАБОТЧИК",
    body: "Хостинг Vercel не поддерживает Sqlite. 'SQLite нужна локальная файловая система на сервере для постоянного хранения данных при выполнении запросов на запись. В среде без сервера это центральное единое постоянное хранилище недоступно, поскольку хранилище является эфемерным с функциями без сервера.'",
  },
  {
    title: "Матвей",
